const colors = {
  ...
  fushsia: {
    fushsia50: 'oklch(0.977 0.017 320.058)',
    fushsia100: 'oklch(0.952 0.037 318.852)',
    fushsia200: 'oklch(0.903 0.076 319.62)',
    fushsia300: 'oklch(0.833 0.145 321.434)',
    fushsia400: 'oklch(0.74 0.238 322.16)',
    fushsia500: 'oklch(0.667 0.295 322.15)',
    fushsia600: 'oklch(0.591 0.293 322.896)',
    fushsia700: 'oklch(0.518 0.253 323.949)',
    fushsia800: 'oklch(0.452 0.211 324.591)',
    fushsia900: 'oklch(0.401 0.17 325.612)',
    fushsia950: 'oklch(0.293 0.136 325.661)'
  },
  pink: {
...
const alphaSteps = [90, 80, 70, 60, 50, 40, 30, 20, 10] as const

type AlphaSteps = (typeof alphaSteps)[number]

type AlphaColorsFor<T extends Shade> = Record<
  `${T}${LightnessSteps}/${AlphaSteps}` | `${T}${LightnessSteps}`,
  string
>

type AlphaColors = {
  [T in Shade]: AlphaColorsFor<T>
}

const generateAlphaColors = (values: typeof colors) => {
  const transformed = Object.entries(values).map(([shade, shades]) => {
    const shadeMap = Object.entries(shades)
      .map(([name, value]) => {
        const colorMap = alphaSteps
          .map(alpha => {
            const colorKey = `${name}/${alpha}`
            return [
              colorKey,
    ...
    return [shade, shadeMap]
  })
  return Object.fromEntries(transformed) as unknown as AlphaColors
}

export const tailwindColors = generateAlphaColors(colors)
